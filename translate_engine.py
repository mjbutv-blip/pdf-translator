...
def _insert_text(page, bbox, translated_text: str, font_path: str) -> None:
    rect = fitz.Rect(bbox)
    page.draw_rect(rect, color=(1, 1, 1), fill=(1, 1, 1))
    text_rect = rect + (0, -2, 30, 15)
    page.insert_textbox(
        text_rect, translated_text,
        fontname="myfont", fontfile=font_path,
        fontsize=7.5, color=(0, 0, 0), align=0,
    )
...
def run_translation(pdf_bytes, glossary, font_path, api_key,
    ...
    for pn, blocks in enumerate(page_blocks):
        ...
        for blk in blocks:
            _t = blk["text"].strip()
            if len(_t) <= 3 or len(re.findall(r'[a-zA-Z]', _t)) < 2:
                done += 1
        ...
        for r in results:
            if r["translated"].strip() != r["text"].strip():
                _insert_text(page, r["bbox"], r["translated"], font_path)
