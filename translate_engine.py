...
SYSTEM_PROMPT = (
    "你是一名专业服装行业翻译，专门处理服装工艺单上的英文文本，将其翻译成地道的中文服装术语。\n\n"
    "【核心身份规则】你的唯一任务是翻译，任何情况下都不得拒绝翻译或原样返回英文。\n\n"
    "【混合数据处理——最高优先级规则】\n"
    "你现在处理的文本通常是表格的一整行，经常将【英文描述】和【多个数字】混在一起"
    "（例如：『front underbust width 30 31 33』）。"
    "你【必须】将里面的英文字母全部翻译成中文，同时【严格按原样保留】所有的数字、符号和它们之间的空格顺序！"
    "例如输入『front width  30  31』，你必须输出『前胸宽  30  31』。"
    "绝对禁止原样返回英文名词！如果遇到混合数据，只翻译英文，不碰数字。\n\n"
    "【输出格式】只返回 JSON，不得有多余文字或 markdown 代码块。\n"
...
def _translate_block(client, text: str, glossary: dict) -> dict:
    ...
    if rel:
        ...
        gloss_block = f"强制术语对照（务必照搬，不得改动）：\n{lines}\n\n"
    ...
    prompt = (
        ...
        "2. 只返回 JSON，不得有多余文字或 markdown 代码块。\n"
        "3. 将对照表里未收录的服装/纺织专业英文词汇放入 unrecorded_terms。\n"
        "4. 待翻译文本大多是服装工艺单上的简短标签、部位名称或材质说明"
        "（如 Weight, Quality, front gore width 等），请务必将所有英文名词翻译成中文，绝不能原样返回英文。\n"
        "5. 【最重要】文本中可能包含尺寸数字或代号（如 30, 20.5, 70A 等）。"
        "你必须在翻译名词的同时，将所有原始数字、标点和尺码原封不动地保留在翻译结果中！绝不能漏掉任何数字！\n"
    )
...
def run_translation(pdf_bytes, glossary, font_path, api_key,
    ...
    if not font_available:
        st.error("⚠️ 找不到中文字体文件 font.ttf，翻译结果将跳过写入步骤，返回原文件！")
    ...
    for pn in range(total_pages):
        ...
        for blk in doc[pn].get_text("dict")["blocks"]:
            ...
            if not text:
                continue
            tb.append({"bbox": blk["bbox"], "text": text})
    ...
    for pn, blocks in enumerate(page_blocks):
        ...
        if font_available:
            # 第一步：红框擦除所有需要替换的原文块
            for r in results:
                if r["translated"].strip() != r["text"].strip():
                    page.add_redact_annot(fitz.Rect(r["bbox"]))
            page.apply_redactions()

            # apply_redactions 会清空页面内容流，必须在其之后重新注册字体
            page.insert_font(fontname="myfont", fontfile=font_path)

            # 第二步：在宽裕的隐形边框内写入中文，防止 insert_textbox 静默失败
            for r in results:
                if r["translated"].strip() != r["text"].strip():
                    rect      = fitz.Rect(r["bbox"])
                    safe_rect = rect + (-2, -2, 50, 20)
                    page.insert_textbox(
                        safe_rect, r["translated"],
                        fontname="myfont", fontfile=font_path,
...
with st.sidebar:
    ...
    if st.session_state.get("_active_user") != user:
        for k in ("zip_result", "zip_count", "unrecorded_terms",
                  "terms_saved", "saved_count", "translation_done", "file_errors"):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with st.status(
            ...
            for i, pdf_file in enumerate(pdf_files):
                ...
                raw_bytes = pdf_file.read()   # 提前读取，保底时可直接复用
                try:
                    ...
                    print(f"处理 {pdf_file.name} 出错:\n{err_detail}")
                    file_errors.append((pdf_file.name, str(exc)))
                    st.error(f"⚠️ 处理「{pdf_file.name}」时发生错误：{exc}")
                    # 保底：将原始未翻译文件打入 ZIP，绝不让用户白等
                    out_name = f"{stem}_原文（翻译失败）.pdf"
                    all_results.append((out_name, raw_bytes))
                    st.write(f"&nbsp;&nbsp;&nbsp;&nbsp;⚠️ **{pdf_file.name}** 翻译失败，已将原文塞入 ZIP")
...
if st.session_state.get("translation_done"):
    st.divider()
    for fname, err_msg in st.session_state.get("file_errors", []):
        st.error(f"文件处理失败：{err_msg}", icon="❌")
