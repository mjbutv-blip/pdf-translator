...
def _translate_block(client, text: str, glossary: dict) -> dict:
    ...
    prompt = (
        ...
        "5. 待翻译文本大多是服装工艺单上的简短标签、部位名称或材质说明"
        "（如 Weight, Quality, front gore width 等）。"
        "请务必将所有英文名词、短语翻译成中文，绝不能原样返回英文。\n\n"
...
def _find_fitting_size(rect: fitz.Rect, text: str, initial_size: float,
                       font_path: str, min_size: float = 8.0) -> float:
    """Decrease font size by 10% each step until text fits in rect or min_size is hit."""
    tmp = fitz.open()
    try:
...
def _insert_text(page, bbox, translated_text: str, size: float, font_path: str) -> None:
    rect = fitz.Rect(bbox) + (-2, -2, 10, 5)
    fitting_size = _find_fitting_size(rect, translated_text, size, font_path)
    ...
    for pn, blocks in enumerate(page_blocks):
        ...
        for blk in blocks:
            if (len(blk["text"].strip()) <= 2
                    or re.fullmatch(r"[0-9A-Z/.,+\-\s]+", blk["text"].strip())):
                done += 1
