...
def _find_fitting_size(rect: fitz.Rect, text: str, initial_size: float,
                       font_path: str, min_size: float = 5.0) -> float:
...
def _insert_text(page, bbox, original_text: str, translated_text: str,
                 size: float, font_path: str) -> None:
    bilingual = f"{original_text}\n{translated_text}"
    rect = fitz.Rect(bbox)
    rect.y1 += rect.height * 1.5
...
def run_translation(pdf_bytes, glossary, font_path, api_key,
    ...
    for pn, blocks in enumerate(page_blocks):
        ...
        for r in results:
            _insert_text(page, r["bbox"], r["text"], r["translated"], r["size"], font_path)
