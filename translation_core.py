...
from db import DbConnection, get_db_connection
...
def _postgres_create_schema(conn: DbConnection) -> None:
    conn.executescript(
        ...
        CREATE TABLE IF NOT EXISTS translation_candidate_occurrences (
            occurrence_id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            translation_job_id TEXT NOT NULL REFERENCES translation_jobs(job_id) ON DELETE CASCADE,
            candidate_id BIGINT NOT NULL REFERENCES term_candidates(candidate_id),
            occurrence_count INTEGER NOT NULL DEFAULT 1,
            representative_page_or_sheet TEXT DEFAULT '',
            representative_cell_coordinate TEXT DEFAULT '',
            original_term_snapshot TEXT NOT NULL,
            normalized_term_snapshot TEXT NOT NULL,
            context_sentence_snapshot TEXT DEFAULT '',
            created_at TEXT NOT NULL,
    ...
    for sql in [
        ...
        "ALTER TABLE translation_jobs ADD COLUMN IF NOT EXISTS execution_mode TEXT",
        "ALTER TABLE translation_jobs ADD COLUMN IF NOT EXISTS worker_id TEXT",
        "ALTER TABLE translation_jobs ADD COLUMN IF NOT EXISTS heartbeat_at TEXT",
        "ALTER TABLE translation_jobs ADD COLUMN IF NOT EXISTS attempt_count INTEGER NOT NULL DEFAULT 0",
    ]:
        conn.execute(sql)
    for sql in [
        """
        ...
        """,
        """
        CREATE INDEX IF NOT EXISTS idx_translation_candidate_occurrences_candidate
        ON translation_candidate_occurrences(candidate_id)
        """,
        """
        ...
        """,
        """
        CREATE INDEX IF NOT EXISTS idx_translation_jobs_pdf_queue
        ON translation_jobs(job_type, status, execution_mode, created_at)
        """,
        """
        CREATE INDEX IF NOT EXISTS idx_translation_jobs_pdf_external_queue
        ON translation_jobs(job_type, status, execution_mode, created_at)
        """,
        """
    ...
    with get_db_connection() as conn:
        ...
        conn.executescript(
            ...
            CREATE TABLE IF NOT EXISTS translation_candidate_occurrences (
                ...
                created_at TEXT NOT NULL,
                UNIQUE(translation_job_id, candidate_id),
                FOREIGN KEY(translation_job_id) REFERENCES translation_jobs(job_id) ON DELETE CASCADE,
        ...
        for col_name, col_sql in [
            ...
            ("execution_mode", "ALTER TABLE translation_jobs ADD COLUMN execution_mode TEXT"),
            ("worker_id", "ALTER TABLE translation_jobs ADD COLUMN worker_id TEXT"),
            ("heartbeat_at", "ALTER TABLE translation_jobs ADD COLUMN heartbeat_at TEXT"),
            ("attempt_count", "ALTER TABLE translation_jobs ADD COLUMN attempt_count INTEGER NOT NULL DEFAULT 0"),
        ]:
...
def save_term_candidates(
    ...
    if not merged:
        return []
    return _persist_term_candidates(
...
def _upsert_translation_candidate_occurrence(
    conn: DbConnection,
    ...
    conn.execute(
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT(translation_job_id, candidate_id) DO UPDATE SET
            occurrence_count = excluded.occurrence_count,
            representative_page_or_sheet = excluded.representative_page_or_sheet,
            representative_cell_coordinate = excluded.representative_cell_coordinate,
            original_term_snapshot = excluded.original_term_snapshot,
            normalized_term_snapshot = excluded.normalized_term_snapshot,
            context_sentence_snapshot = excluded.context_sentence_snapshot
        """,
        (
            translation_job_id,
            candidate_id,
            int(item.get("frequency", 1) or 1),
            str(item.get("page_or_sheet", "") or ""),
            str(item.get("cell_coordinate", "") or ""),
    ...
    with get_db_connection() as conn:
        for key, item in merged_candidates.items():
            ...
            if existing:
                ...
                returning = " RETURNING candidate_id" if conn.is_postgres else ""
                cur = conn.execute(
                    f"""
            ...
            if translation_job_id:
                _upsert_translation_candidate_occurrence(
                    conn,
                    translation_job_id=translation_job_id,
...
def _unrecorded_terms_from_candidate_contexts(candidate_contexts: list[dict]) -> list[dict]:
    seen: set[str] = set()
    rows: list[dict] = []
    for item in candidate_contexts:
        term = str(item.get("term", "")).strip()
        ...
        rows.append({
            "original_term": term,
            "normalized_term": key,
            "sheet": str(item.get("page_or_sheet", "")).strip(),
            "cell_coordinate": str(item.get("cell_coordinate", "")).strip(),
            "context": str(item.get("context", "")).strip(),
        })
    return rows
...
def _build_excel_review_summary(
    customer_id: str | None,
    candidate_contexts: list[dict],
    glossary: dict,
) -> dict:
    unrecorded_terms = _unrecorded_terms_from_candidate_contexts(candidate_contexts)
    ...
    review_summary = _build_translation_review_summary(len(unrecorded_terms), candidate_stats)
    review_summary["unrecorded_term_count"] = len(unrecorded_terms)
    review_summary["unrecorded_terms"] = unrecorded_terms
    review_summary["image_translation_unrecorded_tracking_supported"] = False
...
def _apply_approved_glossary_change(conn: DbConnection, request_row) -> None:
    ...
    if action == "add":
        ...
        if cur.rowcount != 1:
            raise ValueError("原 active 术语不存在或已被处理，无法应用修改申请")
    elif action == "delete":
        cur = conn.execute(
            """
            UPDATE glossary_terms
        ...
        if cur.rowcount != 1:
            raise ValueError("原 active 术语不存在或已被处理，无法应用删除申请")
...
def run_pdf_translation(
    ...
    review_summary["unrecorded_term_count"] = len(all_unrecorded)
    review_summary["unrecorded_terms"] = [
        {"original_term": term}
        for term in sorted(all_unrecorded)
    ]
...
def run_excel_translation(
    ...
    def persist_candidates_once() -> None:
        if customer_id and source_file_name and created_by:
            save_term_candidates(
                customer_id=customer_id,
                source_file_name=source_file_name,
    ...
    if not translate_images:
        persist_candidates_once()
        on_progress(1.0)
        review_summary = _build_excel_review_summary(customer_id, candidate_contexts, glossary)
